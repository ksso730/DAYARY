...
public interface MoimBoardRepository extends JpaRepository<MoimBoard, Long> {
	...
	@Query(value = "select m from MoimBoard m inner join  MoimBoardFile f WHERE m.moim=(:moim) and  m.boardGroup=(:boardGroup) and f.representImage = (:representImage)",countQuery = "select count(m) from MoimBoard m")
